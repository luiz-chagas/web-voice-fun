...
import { initializeVoiceController } from "../controllers/voice";
...
export class GameScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    const voiceController = initializeVoiceController({
      onSlow: () => {
        this.timeout *= 2;
        setTimeout(() => {
          this.timeout /= 2;
    ...
    this.player = new Snake(this, voiceController);
  ...
  update(time: number): void {
    ...
    if (!this.player.isDead()) {
      if (time - this.tick > this.timeout) {
        this.player.move();
        this.checkCollision();
        this.tick = time;
      }
